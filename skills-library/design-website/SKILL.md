---
name: design-website
description: Generate a premium mockup website for a prospect using a bold editorial template style. Use when user asks to design a website, create a mockup, or build a prospect website.
...
## Goal
Generate a polished, single-page HTML website mockup for a prospect's business. The website uses a bold editorial aesthetic: bold typography, off-white backgrounds, editorial grid layouts, terracotta accents. Used to pitch web design services.
...
```bash
python3 ./scripts/read_prospect.py \
  --url "SHEET_URL" \
  --row ROW_NUMBER \
...
Or with a JSON file:
```bash
python3 ./scripts/generate_website.py < prospect.json
```
