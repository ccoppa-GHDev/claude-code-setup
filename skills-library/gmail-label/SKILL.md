...
### Step 1: Fetch emails
```bash
python3 ./scripts/gmail_label_fetch.py \
  --account ACCOUNT --query "in:inbox" --limit 100 --output .tmp/emails.json
```

### Step 2: Split into chunks
```bash
python3 ./scripts/gmail_label_split.py \
...
```bash
python3 ./scripts/gmail_label_merge.py \
  --input-dir .tmp/chunks --output .tmp/labels.json
```

### Step 5: Apply labels
```bash
python3 ./scripts/gmail_label_apply.py \
...
## Adding New Accounts
```bash
python3 ./scripts/gmail_multi_auth.py --account ACCOUNT_NAME --email EMAIL
```
